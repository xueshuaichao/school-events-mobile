...
if [[ $ENV = 'online' ]]; then
    ...
    docker build -t dockerhub.wdcloud.cc/school-events-mobile-fe-${ENV}:latest -f deploy/Dockerfile .
    docker push  dockerhub.wdcloud.cc/school-events-mobile-fe-${ENV}:latest
fi
