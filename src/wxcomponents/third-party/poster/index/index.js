...
const handle = {
    ...
    _getTextWidth(text) {
        ...
        texts.forEach(({ fontSize, text, marginLeft = 0, marginRight = 0 }) => {
            ...
            width +=
                this.ctx.measureText(text).width + marginLeft + marginRight;
        });
    ...
    _drawSingleText({
        ...
        this.ctx.save();
        this.ctx.beginPath();
        this.ctx.font = `${fontStyle} ${fontWeight} ${this.toPx(
            fontSize,
            true
        )}px ${fontFamily}`;
        this.ctx.setGlobalAlpha(opacity);
        ...
        if (textDecoration !== "none") {
            ...
            this.ctx.lineTo(
                this.toPx(x) + this.toPx(textWidth),
                this.toPx(lineY)
            );
            this.ctx.setStrokeStyle(color);
            this.ctx.stroke();
    ...
    methods: Object.assign(
        {
            ...
            getHeight(config) {
                ...
                (config.texts || []).forEach(item => {
                    let height;
                    if (
                        Object.prototype.toString.call(item.text) ===
                        "[object Array]"
                    ) {
                        item.text.forEach(i => {
